...
YesFood is a grocery store and sells items such as produce, meat, dairy, baked goods, snacks, and other household food staples.
As food costs rise, YesFood wants to make sure it keeps stocking products in all categories that cover a range of prices to ensure they have stock for a broad range of customers. 
...
Here's the query to determine how many products have the `year_added` value missing. The output given below as `missing_year`, with a single row giving the number of missing values.
